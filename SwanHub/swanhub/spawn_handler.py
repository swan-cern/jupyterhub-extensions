...
class SpawnHandler(JHSpawnHandler):
    ...
    async def get(self, for_user=None, server_name=""):
        ...
        if spawner.ready:
            ...
            next_url = url_path_join(
                "user",
                user.escaped_name,
                "customenvs" if spawner_software_source == configs.customenv_special_type else "",
            )
    ...
    async def _post(self, user_name, server_name):
        ...
        if spawner.ready:
            ...
            raise web.HTTPError(400, f"{spawner._log_name} is pending {spawner.pending}")
        ...
        try:
            ...
            if spawner._spawn_future and not spawner._spawn_future.done():
                ...
                def _finish_spawn(f):
                    ...
                    if f.exception() is None:
                        ...
                        self.log.error("Failed to spawn single-user server", exc_info=True)
            ...
            if type(e) in (web.HTTPError, TimeoutError):
                ...
                self.log.warning("Failed to spawn single-user server with known error", exc_info=True)
            else:
                error_message = str(e)
        ...
        if options.get(configs.software_source) == configs.customenv_special_type:
            ...
            if options.get(configs.builder_version):
                ...
                query_params[configs.builder], query_params[configs.builder_version] = options[
                    configs.lcg_rel_field
                ].split("-")
            ...
            next_url = self.get_next_url(
                user, default=url_path_join(self.hub.base_url, "spawn-pending", user.escaped_name, server_name)
            )
            if options[configs.use_jupyterlab_field] == "checked":
                ...
                # User requested to open a file
                    ...
                    next_url = url_path_join(
                        "user", user.escaped_name, "lab", "tree", *options[configs.file].split("/")
                    )
    ...
    async def _render_form(self, for_user, spawner_options_form, message="", *args, **kwargs):
        ...
        return await self.render_template(
            ...
            url=url_concat(self.request.uri, {"_xsrf": self.xsrf_token.decode("ascii")}),
    ...
    def _log_spawn_metrics(self, user, options, spawn_duration_sec, spawn_exception=None):
        ...
        if not spawn_exception:
            ...
            self._log_metric(
                user.name, host, ".".join(["spawn", spawn_context_key, "exception_class"]), spawn_exc_class
            )
            self._log_metric(
                user.name, host, ".".join(["spawn", spawn_context_key, "duration_sec"]), spawn_duration_sec
            )
        else:
