...
class BinderSpawnerMixin(Configurable):
    ...
    def start(self):
        if not self.auth_enabled:
            # these two web.HTTPErrors are removed from the original
            # mixin class in order to be able to start 'normal' containers
            if "token" not in self.user_options:
                self.log.info("no token is set, probably not a binder container")
                # raise web.HTTPError(400, "token required")
            if "image" not in self.user_options:
                self.log.info("no image is set, probably not a binder container")
