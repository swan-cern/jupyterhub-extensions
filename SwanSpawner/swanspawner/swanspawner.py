    ...
    class SwanSpawner(base_class):
        ...
        def _get_selection(self, options_form_config: dict, options: dict, parent: str) -> dict:
            ...
            selection = next(
                (
                    _
                    for _ in options_form_config[f"{options[self.software_source]}_options"]
                    if _["type"] == "selection" and options[parent] == _[parent]["value"]
                ),
                None,
            )
            if not selection:
        ...
        def _validate_selection_options(self, selection: dict, options: dict) -> None:
            ...
            for attr, available_options in selection.items():
                ...
                elif type(available_options) == list and options.get(attr) not in (
                    _.get("value") for _ in available_options
                ):
        ...
        def _validate_rucio_options(self, selection: dict, formdata: dict) -> dict:
            ...
            selected_rucio_inst = next((inst for inst in rucio_instances if inst["value"] == rucio_instance), None)
            ...
            selected_rse_config = next((rse for rse in rse_options if rse["value"] == selected_rse), None)
            ...
            if form_mount_path and form_mount_path != rucio_options[self.rucio_rse_mount_path]:
                self.log.warning(
                    f"Mount path mismatch: form={form_mount_path}, expected={rucio_options[self.rucio_rse_mount_path]}"
                )
        ...
        def options_from_form(self, formdata: dict) -> dict:
            ...
            if options[self.software_source] == self.customenv_special_type:
                ...
                if options[self.lcg_rel_field].split("-")[0] in self.stacks_for_customenvs:
                    ...
                    options[self.user_interface] = (
                        "lab" if options[self.use_jupyterlab_field] == "checked" else "projects"
                    )
            else:
        ...
        def get_env(self):
            ...
            if self.user_options[self.use_jupyterlab_field] == "checked":
                env.update(dict(SWAN_USE_JUPYTERLAB="true"))
        ...
        async def stop(self, now=False):
            ...
            self.log_metric(self.user.name, self.this_host, "exit_container_code", container_exit_code)
        ...
        async def poll(self):
            ...
            if container_exit_code is not None:
                ...
                self.log_metric(self.user.name, self.this_host, "exit_container_code", value_cleaned)
        ...
        async def start(self):
            ...
            self.log_metric(
                self.user.name, self.this_host, "start_container_duration_sec", time.time() - start_time_start_container
            )
