...
def define_SwanSpawner_from(base_class):
    ...
    class SwanSpawner(base_class):
        ...
        def _validate_selection_options(self, selection: dict, options: dict) -> None:
            ...
            # Same for 'rucio' which is a nested configuration block
            skip_attrs = {'rucio', 'platforms'}
        ...
        def _validate_rucio_options(self, selection: dict, formdata: dict) -> dict:
            ...
            rucio_instance = formdata.get(self.rucio_instance, ['none'])[0]
            ...
            form_mount_path = formdata.get(self.rucio_rse_mount_path, [''])[0]
            form_path_begins = formdata.get(self.rucio_path_begins_at, ['0'])[0]
        ...
        def options_from_form(self, formdata: dict) -> dict:
            ...
            if options[self.software_source] == self.customenv_special_type:
                ...
                # Validate and extract Rucio options (if Rucio is configured in the YAML)
                if 'rucio' in selection:
                    rucio_opts = self._validate_rucio_options(selection, formdata)
                    options.update(rucio_opts)
                else:
                    # No Rucio configuration in YAML, set defaults
                    options[self.rucio_instance] = 'none'
                    options[self.rucio_rse] = 'none'
                    options[self.rucio_rse_mount_path] = ''
                    options[self.rucio_path_begins_at] = '0'
            else:
