...
class AvailableGPUs:
    ...
    def _update_free_gpu_flavours(self) -> None:
        ...
        try:
            ...
            user_gpu_pods = [
                pod
                for pod in self._api.list_namespaced_pod(namespace=self.NAMESPACE, label_selector="gpu").items
            ...
            for pod in user_gpu_pods:
                ...
                if container is None:
                    raise RuntimeError(
                        f"_update_free_gpu_flavours: Expected container named '{self.CONTAINER_NAME}' not found in pod.spec.containers"
                    )
            ...
            with self._lock:
                for description, gpu_info in self._gpus.items():
                    ...
                    self._logger.info(
                        f"{description} -> Used: {used_count}, Total: {gpu_info.count}, Free: {gpu_info.free}"
                    )

            self._logger.info(
                "Final currently free GPU state: "
                + ", ".join([f"{desc}: {info.free}/{info.count}" for desc, info in self._gpus.items()])
            )
    ...
    def get_available_gpu_flavours(self, roles: list[str]) -> dict:
        ...
        try:
            ...
            with self._lock:
                return {name: gpu for name, gpu in self._gpus.items() if name in allowed_flavors}
    ...
    def get_free_gpu_flavours(self, roles: list[str]) -> dict:
        ...
        return {name: gpu for name, gpu in available_flavours.items() if gpu.free > 0}
    ...
    def _update_allocatable_gpu_flavours(self) -> None:
        ...
        for node in all_gpu_nodes:
            ...
            if mig_config == "all-disabled":
                ...
                self._process_full_card(
                    gpus, gpu_model, product_name, node_to_flavor, status, labels, node.metadata.name
                )
            else:
        ...
        self._logger.info(
            "Allocatable GPU flavours and their counts: "
