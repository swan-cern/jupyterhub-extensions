        ...
        try:
            ...
            with self._lock:
                for (node_name, resource_name), description in self._node_to_flavor.items():
                    ...
                    has_lhcb_role = node_name in lhcb_nodes
                    has_events_role = node_name in events_nodes
                    ...
                    if is_swan_events:
                        ...
                        if not has_events_role:
                            allowed_flavors.add(description)
                    elif is_normal:
                        # Normal users see all except lhcb and events nodes
                        if not has_lhcb_role and not has_events_role:
                            allowed_flavors.add(description)
            
        except ApiException as e:
            self._logger.error(f'Error getting node labels for role filtering: {e}')
